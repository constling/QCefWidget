...
void CefWnd::setupUi() {
  ...
  if (usingGLWidget_) {
    ...
    pCefWidget_ = new QCefWidget(initUrl_, this);
    pCefWidget_->setGeometry(0, 0, 600, 600);
    pCefWidget_->setObjectName("cefWidget");
    pCefWidget_->setOsrEnabled(false);
    pCefWidget_->setContextMenuEnabled(false);
    pCefWidget_->setAutoShowDevToolsContextMenu(false);
    //pCefWidget_->setBrowserBackgroundColor(browserBkColor_);
    pCefWidget_->setAllowExecuteUnknownProtocolViaOS(false);
    pCefWidget_->setAutoDestoryCefWhenCloseEvent(false);
